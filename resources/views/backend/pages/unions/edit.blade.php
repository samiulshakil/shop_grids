...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
            <form action="{{ route('admin.unions.update', $union->id) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-12">
                        <div class="main-card mb-3 card">
                            <div class="card-body">
                                    ...
                                    <select onchange="zilaList(this.value)" required
                                        ...
                                        @forelse ($divisions as $division)
                                            <option value="{{ $division->id }}"
                                                @if ($union->division_id == $division->id) selected @endif>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" id="name" value="{{ $union->name }}" name="name"
                                ...
                                <div class="form-group">
                                    <div class="custom-control custom-switch">
                                        <input type="checkbox" name="status" class="custom-control-input" id="status"
                                            @if ($union->status == true) checked @endif>
...
@push('js')
    <script>
        $(document).ready(function() {
            ...
            let division_id = {{ $union->division_id }}
            $.ajax({
                ...
                success: function(data) {
                    $('#zila_id').html('');
                    $('#zila_id').html(data);
                    $('#zila_id option[value="' + {{ $union->zila_id }} + '"]').prop(
                        'selected', true);
                },
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#upazila_id option[value="' + {{ $union->upazila_id }} + '"]').prop(
                        'selected', true);
                },
                error: function(xhr, ajaxOption, thrownError) {
                    console.log(thrownError + '\r\n' + xhr.statusText + '\r\n' + xhr.responseText);
